...
# In[Wasserstein_Multi_Robot_Planner]
run = 0
if run: 
    timeout = 10000
    planner = Wasserstein_Multi_Robot_Planner(xc, h0, h1, num_robot=1, pos_x_list=None, dW2=1, 
...
if run: 
    timeout = 10000
    planner = Wasserstein_Multi_Robot_Planner(xc, h0, h1, num_robot=5, pos_x_list=None, dW2=1, 
                                              apply_steepness_constraint=True, apply_modification_noise=True, 
                                              log_actions=False, log_progress=False, log_construction_state=True) 
    # planner.set_video(frame_sampling_period=10)
    planner.solve(timeout=timeout) 
...
if run: 
    timeout = 10000
    planner = Flatten_Wasserstein_Planner(xc, h0, h1, num_robot=5, steepness_desired=np.array([-0.3, 0.3]), dW2=1, 
                                          apply_steepness_constraint=True, apply_modification_noise=True, 
                                          log_actions=False, log_progress=False, log_construction_state=True)
    # planner.set_video(frame_sampling_period=10)
